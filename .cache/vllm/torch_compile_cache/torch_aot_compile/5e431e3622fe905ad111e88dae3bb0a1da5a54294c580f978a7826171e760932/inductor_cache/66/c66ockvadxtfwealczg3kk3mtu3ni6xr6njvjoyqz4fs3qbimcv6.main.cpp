...
static PyObject* kernel_py(PyObject* self, PyObject* args) {
    try {
        ...
        if(unlikely(PyTuple_GET_SIZE(args) != 13))
            throw std::runtime_error("requires 13 args");
        kernel(parse_arg<int32_t*>(args, 0), parse_arg<float*>(args, 1), parse_arg<float*>(args, 2), parse_arg<int64_t*>(args, 3), parse_arg<float*>(args, 4), parse_arg<float*>(args, 5), parse_arg<float*>(args, 6), parse_arg<float*>(args, 7), parse_arg<float*>(args, 8), parse_arg<float*>(args, 9), parse_arg<float*>(args, 10), parse_arg<int32_t*>(args, 11), parse_arg<int64_t>(args, 12)); Py_RETURN_NONE;
